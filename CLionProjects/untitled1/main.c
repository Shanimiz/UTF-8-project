...
char *my_utf8_charat(char *string, int index) {
    ...
    /*iterates through the input string until either the end of the string ('\0') is reached or the specified
     index is 0 */
    while (*string != '\0' && index > 0) {
        unsigned char ch = (unsigned char)*string;

        if ((ch & 0xC0) != 0x80) {
            // checks if the current byte is the start of a new UTF-8 character, with the same logic I used earlier
            index--;
        }
    ...
    if (*string == '\0' || (index > 0 && (string - 1) == '\0')) {
...
int my_utf8_strcmp(unsigned char *string1, unsigned char *string2) {
    while (*string1 != '\0' && *string2 != '\0') {
        unsigned int codePoint1 = 0, codePoint2 = 0;
        int bytes1 = my_utf8_decode(string1, &codePoint1);
        int bytes2 = my_utf8_decode(string2, &codePoint2);
...
int is_overlong_encoding(unsigned char byte, unsigned int code_point) {
    if (code_point <= 0x7F) {
        // One-byte character, should not be overlong
...
void test_all_utf8_checks() {
    ...
    test_utf8_check("Overlong encoding", -3);
...
int main() {
    ...
    // Encoding
    printf("Hey, please enter the input for Encoding: ");
    fgets(input1, sizeof(input1), stdin);
    my_utf8_encode(input1, encodedOutput1);
    printf("Encoded UTF-8 string: %s\nLength: %d characters\n", encodedOutput1, my_utf8_strlen(encodedOutput1));

    // Print the longest continuous sequence
    findLongestContinuousSequence(encodedOutput1);

    // Index retrieval
    int index;
    printf("Enter index to retrieve character from Encoded string: ");
    scanf("%d", &index);
    clearInputBuffer();
    char *result = my_utf8_charat(encodedOutput1, index);
    if (result != NULL) {
        printf("Character at index %d in Encoded string: %.*s\n", index, (int)(result - encodedOutput1), result);
    } else {
    ...
    if (comparisonResult == 0) {
        ...
        printf("Encoded string 1 is greater than Encoded string 2.\n");
    }
    ...
    my_utf8_substring(encodedOutput1, start, length, substringOutput);
    printf("Substring: %s\n", substringOutput);
    ...
    if (validationResult == 0) {
        printf("Encoded UTF-8 string is valid.\n");
    } else {
        printf("Encoded UTF-8 string is not valid (Error code: %d).\n", validationResult);
    }
